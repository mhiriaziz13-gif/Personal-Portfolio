...
- Server-rendered Next.js routes fetch small structured content records; public pages remain lightweight and cacheable by the host.
...
- Analytics scripts are limited to Vercel Analytics, Vercel Speed Insights and the configured Google tag.
- Dependency footprint remains restrained: Next.js, React, TypeScript, Supabase clients and Vercel measurement packages.
- CSS transitions are short and disabled for reduced-motion users.
- Initial visual assets are local; optional uploaded assets are served from the configured storage bucket.
...
5. Confirm both initial `public/cv/` links and newly uploaded CV files return the correct downloads.
