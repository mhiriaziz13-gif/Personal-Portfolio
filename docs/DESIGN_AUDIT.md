...
- **Maintainability:** public content is structured, reusable and supported by local fallbacks; repeated UI patterns are components, and CV files can be managed from the private area or regenerated from the documented Python source.

## Longevity

The site relies on structure, typography, contrast and content hierarchy instead of short-lived visual effects. New projects, roles, skills and CV links can be managed from the private area without changing routes; visual tokens remain centralised in `app/globals.css`.